...
export const MarkdownProvider = ({
  ...
  useEffect(() => {
    ...
    const startsWithSingleAsterisk =
      selectedText.startsWith("*") && !selectedText.startsWith("**");
    const endsWithSingleAsterisk =
    ...
    const endsWithSingleUnderscore =
  ...
  const insertMarkdown = (format: string) => {
      ...
      switch (format) {
        ...
        case "italic":
          ...
          if (/^\*\*\*.*\*\*\*$/.test(selectedText)) {
            ...
            italicStartsWithSingleUnderscore &&
            italicEndsWithSingleUnderscore
          ) {
            shouldRemove = true;
            insertion = selectedText.slice(1, -1);
          }
    ...
    setTimeout(() => {
      ...
      if (start !== end || shouldRemove) {
        ...
        const updatedSelectedText = textarea.value.substring(
          newStartPos,
          newEndPos,
        );
  ...
  const deleteTableColumn = (colIndex: number) => {
    if (tableCols > 1) {
      setTableCols(tableCols - 1);
      setTableContent(
        tableContent.map((row) => row.filter((_, index) => index !== colIndex)),
      );
    }
  };
  ...
  const insertMarkdownTable = () => {
    ...
    if (!textarea) {
      ...
      setMarkdown(
        (prevMarkdown: string) => prevMarkdown + "\n\n" + markdownTable,
      );
      return;
    }

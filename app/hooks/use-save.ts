...
export function useSave<T = unknown>() {
  ...
  const [dialogContent, setDialogContent] = useState<SaveDialogContent | null>(null);
      ...
      if (config.onSuccess) {
        if (config.onSuccess.redirectPath) {
          setDialogContent({
            ...
            description: config.onSuccess.redirectPrompt || config.onSuccess.description,
            type: 'confirm',
  ...
  const handleError = async (res: Response, customErrorMessages?: Record<string, string>) => {
    try {
